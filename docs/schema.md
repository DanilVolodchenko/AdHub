...
2. В случае ошибки с токеном возврат ответа с кодом HTTP 401 (UNAUTHORIZED) и сообщением 'Неверный токен!'.
3. Возврат ответа в формате JSON с кодом HTTP 200 (OK) и информацией о текущем пользователе.
...
```
{
  "content": "Изменение роль пользователя user прошло успешно!",
  "status_code": 200
}
```
...
- ad:
    - Тип: AdCreateSchema
    - Описание: Объект с данными для создания объявления.

### Ход выполнения

1. Попытка получить информацию о текущем пользователе с использованием переданного токена.
2. В случае ошибки с токеном возврат ответа с кодом HTTP 401 и сообщением 'Неверный токен!'.
3. Создание нового объявления в базе данных, используя переданные данные (заголовок, описание, id владельца).
...
```
{
  "content": "Комментарий успешно создан!",
  "status_code": 201
}
```
